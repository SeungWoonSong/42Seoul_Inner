...
void	execute_last(t_pipe_data *data, char **envp)
{
	...
	if (data->pid[data->total - data->num_command] == 0)
	{
		...
		execve(data->operand[data->total - 1],
			data->option[data->total - 1], envp);
	}
	else
		return ;
}
...
void	execute_middle(t_pipe_data *data, char **envp)
{
	...
	if (data->pid[data->total - data->num_command] == 0)
	{
		...
		execve(data->operand[data->total - data->num_command],
			data->option[data->total - data->num_command], envp);
	}
	else
		return ;
}
